...
public class base19_regions extends StarMacro {
  ...
  private void execute0() {
    ...
    simulation_0.getRegionManager().newRegionsFromParts(new NeoObjectVector(new Object[] {solidModelPart_6, meshOperationPart_4}), "OneRegionPerPart", null, "OneBoundaryPerPartSurface", null, "OneFeatureCurve", null, RegionManager.CreateInterfaceMode.BOUNDARY);
    ...
    String frontRight = "Aero wrap (Non-aero wrap, wings, mounts and bodywork).Non-aero wrap (Car).Front Right.Default";
    String rearRight = "Aero wrap (Non-aero wrap, wings, mounts and bodywork).Non-aero wrap (Car).Rear Right.Default";
    ...
    for (Boundary check : bounds)
    {
      ...
      if (boundName.startsWith("Front Left", 74 ))
        ...
        rearLeft = boundName;
      else if (boundName.startsWith("Front Right", 74))
      {
        frontRight = boundName;
        fullCar = 1;
      }
      else if (boundName.startsWith("Rear Right", 74))
        rearRight = boundName;
    }
    ...
    if (fullCar == 1)
    {
      ...
      Boundary frontRightBound = region_7.getBoundaryManager().getBoundary(frontRight);
      frontRightBound.setPresentationName("Front Right Wheel");
      Boundary rearRightBound = region_7.getBoundaryManager().getBoundary(rearRight);
      rearRightBound.setPresentationName("Rear Right Wheel");

      frontRightBound.getConditions().get(WallSlidingOption.class).setSelected(WallSlidingOption.Type.LOCAL_ROTATION_RATE);

      ReferenceFrame referenceFrame_0 =
              frontRightBound.getValues().get(ReferenceFrame.class);
      ...
      WallRelativeRotationProfile wallRelativeRotationProfile_0 =
              frontRightBound.getValues().get(WallRelativeRotationProfile.class);

      wallRelativeRotationProfile_0.getMethod(ConstantScalarProfileMethod.class).getQuantity().setValue(65.617);

      rearRightBound.getConditions().get(WallSlidingOption.class).setSelected(WallSlidingOption.Type.LOCAL_ROTATION_RATE);

      ReferenceFrame referenceFrame_1 =
              rearRightBound.getValues().get(ReferenceFrame.class);
      ...
      WallRelativeRotationProfile wallRelativeRotationProfile_1 =
              rearRightBound.getValues().get(WallRelativeRotationProfile.class);
